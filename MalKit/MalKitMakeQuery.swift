...
class MalKitMakeQuery{
    ...
    static let min_value_s: Int = 1
    static let max_value_s: Int = 6
    static let min_value_e: Int = 0
    static let max_value_e: Int = 1
    ...
    class func makeAnimeQuerty(query: Array<Any?>, type: Int) -> String {
        ...
        if type == MalKitGlobalVar.RequestType.Update.rawValue {
            if query[1] != nil{
                str += "<status>"
                str += String(min(max(query[1] as! Int, min_value_s), max_value_s))
                str += "</status>"
            }
        }else{
            str += "<status>"
        ...
        if query[10] != nil{
            ...
            str += String(min(max(query[10] as! Int, min_value_e), max_value_e))
            str += "</enable_discussion>"
        }
        
        if query[11] != nil{
            str += "<enable_rewatching>"
            str += String(min(max(query[11] as! Int, min_value_e), max_value_e))
            str += "</enable_rewatching>"
        }
    ...
    class func makeMangaQuerty(query: Array<Any?>, type: Int) -> String {
        ...
        if type == MalKitGlobalVar.RequestType.Update.rawValue {
            if query[2] != nil{
                str += "<status>"
                str += String(min(max(query[2] as! Int, min_value_s), max_value_s))
                str += "</status>"
            }
        }else{
            str += "<status>"
        ...
        if query[9] != nil{
            str += "<enable_discussion>"
            str += String(min(max(query[9] as! Int, min_value_e), max_value_e))
            str += "</enable_discussion>"
        }
